...
export async function fetchFacebookUserProfile(pageId: string, psid: string) {
  const token = (await getPageAccessToken(pageId)) || process.env.FB_PAGE_ACCESS_TOKEN;
...
export async function sendTextMessageViaGraph(pageId: string, psid: string, text: string) {
  ...
  if (!r.ok) {
    const msg = data?.error?.message || `Facebook send failed (HTTP ${r.status})`;
